...
docker run --rm --net demo-kanata-drupal-cms_default amazeeio/dockerize dockerize -wait tcp://mariadb:3306 -timeout 1m
```
...
docker ps --filter label=com.docker.compose.project=demo-kanata-drupal-cms | grep Up | grep demo-kanata-drupal-cms-nginx-1
docker ps --filter label=com.docker.compose.project=demo-kanata-drupal-cms | grep Up | grep demo-kanata-drupal-cms-mariadb-1
docker ps --filter label=com.docker.compose.project=demo-kanata-drupal-cms | grep Up | grep demo-kanata-drupal-cms-php-1
docker ps --filter label=com.docker.compose.project=demo-kanata-drupal-cms | grep Up | grep demo-kanata-drupal-cms-cli-1

# Should ssh against the cli container by default
docker compose exec -T cli bash -c "env | grep LAGOON=" | grep cli-drupal

# Should have the correct environment set
docker compose exec -T cli bash -c "env" | grep LAGOON_ROUTE | grep demo-kanata-drupal-cms.docker.amazee.io
